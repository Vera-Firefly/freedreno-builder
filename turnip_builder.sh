...
patch="https://github.com/Vera-Firefly/freedreno-builder/archive/refs/heads/patch-1.zip"
clear
...
prepare_workdir(){
	...
	curl "$mesasrc" --output mesa-mesa-24.1.2.zip &> /dev/null
	###
    ...
    cp -rf patch-1/freedreno_devices.py mesa-mesa-24.1.2/src/freedreno/common/
...
EOF
	...
	meson build-android-aarch64 --cross-file "$workdir"/mesa-mesa-24.1.2/android-aarch64 -Dbuildtype=release -Dplatforms=android -Dplatform-sdk-version=$sdkver -Dandroid-stub=true -Dgallium-drivers= -Dvulkan-drivers=freedreno -Dvulkan-beta=true -Dfreedreno-kmds=kgsl -Db_lto=true &> "$workdir"/meson_log
...
port_lib_for_magisk(){
	...
	cp "$workdir"/mesa-mesa-24.1.2/build-android-aarch64/src/freedreno/vulkan/libvulkan_freedreno.so "$workdir"
	cd "$workdir"
